...
class RAGService:
    ...
    def query(
        ...
        model_name = assistant_config.get("llm_model") if assistant_config else None
        ...
        if not kb_ids:
            ...
            answer = self.llm_client.generate_general_response(query_text, context, model_name=model_name)
        ...
        if settings.ENABLE_MULTI_HOP and analysis.get("is_multi_hop"):
            result = self._multi_hop_query(query_text, analysis.get("sub_queries", []), top_k, history_str, kb_ids, system_prompt, model_name=model_name)
        else:
            result = self._single_hop_query(query_text, top_k, history_str, kb_ids, system_prompt, model_name=model_name)
    ...
    def _single_hop_query(
        ...
        回答："""
             answer, first_token, total_time = self.llm_client.generate_response_with_metrics(prompt, model_name=model_name)
            ...
            answer = self.llm_client.generate_response(query_text, context, model_name=model_name)
    ...
    def _multi_hop_query(
        ...
        # Final Answer
        answer = self.llm_client.generate_response(query_text, final_context, model_name=model_name)
